...
class SlavePort(core_ports.BasePort):
    ...
    _DEVICE_EXPRESSION_ATTRDEF = {"type": "string", "modifiable": True, "max": 1024}
    ...
    _LAST_SYNC_ATTRDEF = {"type": "number", "modifiable": False}

    _EXPIRES_ATTRDEF = {"type": "number", "modifiable": True, "min": 0, "max": 2147483647}
    ...
    async def set_sequence(self, values: list[PortValue], delays: list[int], repeat: int) -> None:
        ...
        try:
            await self._slave.api_call(
                "PATCH", f"/ports/{self._remote_id}/sequence", {"values": values, "delays": delays, "repeat": repeat}
            )
