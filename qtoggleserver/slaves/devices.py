...
class Slave(logging_utils.LoggableMixin):
    ...
    def __eq__(self, s: Slave) -> bool:
        return (
            self._scheme == s.get_scheme()
            and self._host == s.get_host()
            and self._port == s.get_port()
    ...
    async def _rename_ports_persisted_data(self, new_name: str) -> None:
        port_data_list = await persist.query(SlavePort.PERSIST_COLLECTION, fields=["id"])
        my_port_data_list = [d for d in port_data_list if d["id"].startswith(f"{self._name}.")]
    ...
    async def _api_call(
        ...
        headers = {
            ...
            "Authorization": core_api_auth.make_auth_header(
                core_api_auth.ORIGIN_CONSUMER, username="admin", password_hash=self._admin_password_hash
            ),
        }
        ...
        request = HTTPRequest(
            url, method, headers=headers, body=body_str, connect_timeout=timeout, request_timeout=timeout
        )
        ...
        try:
            ...
            if (
                ...
                and (retry_counter < settings.slaves.retry_count)
    ...
    async def _add_port(self, attrs: Attributes) -> core_ports.BasePort:
        ...
        port = await core_ports.load_one(SlavePort, {"slave": self, "attrs": attrs})
        ...
        while True:
            try:
                ...
                try:
                    ...
                    self.error(
                        "api call GET /listen failed: %s, retrying in %s seconds", e, settings.slaves.retry_interval
                    )
    ...
    async def _poll_once(self) -> int:
        ...
        changed_names = [n for n in self._cached_attrs if (n in attrs) and (attrs[n] != self._cached_attrs[n])]
        ...
        if not self._online:
            ...
            if not self._online:
                self.warning(
                    "device did not successfully go online, retrying in %s seconds", settings.slaves.retry_interval
                )
    ...
    async def _handle_value_change(
        self, id: str, value: NullablePortValue, old_value: NullablePortValue = None
    ) -> None:
    ...
    async def _handle_port_update(self, **attrs: Attribute) -> None:
        ...
        for name, value in attrs.items():
            ...
            if old_value is not None and value != old_value:
                self.debug(
                    "%s.%s changed remotely: %s -> %s", port, name, json_utils.dumps(old_value), json_utils.dumps(value)
                )
    ...
    async def apply_provisioning(self) -> None:
        ...
        for port in self._get_local_ports():
            ...
            if attrs:
                ...
                try:
                    await self.api_call(
                        "PATCH", f"/ports/{port.get_remote_id()}", attrs, timeout=settings.slaves.long_timeout
                    )
                except Exception as e:
            ...
            if value is not None:
                ...
                try:
                    await self.api_call(
                        "PATCH", f"/ports/{port.get_remote_id()}/value", timeout=settings.slaves.long_timeout
                    )
                except Exception as e:
...
async def _handle_rename(slave: Slave, new_name: str) -> None:
    ...
    slave = await add(
        scheme, host, port, path, poll_interval=0, listen_enabled=False, admin_password_hash=admin_password_hash
    )
