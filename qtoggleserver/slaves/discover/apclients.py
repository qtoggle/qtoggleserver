...
async def finish() -> None:
    ...
    if _discover_task:
        ...
        try:
            ...
            logger.exception("discover task error")
...
async def configure(discovered_device: DiscoveredDevice, attrs: Attributes) -> DiscoveredDevice:
    ...
    if "admin_password" not in attrs:
        ...
        discovered_device.admin_password = salves_utils.generate_password(
            core_device_attrs.admin_password_hash, discovered_device.ap_client.mac_address, "admin"
        )
        ...
        if "normal_password" in discovered_device.attrs:
            attrs["normal_password"] = salves_utils.generate_password(
                core_device_attrs.admin_password_hash, discovered_device.ap_client.mac_address, "normal"
            )

        if "viewonly_password" in discovered_device.attrs:
            attrs["viewonly_password"] = salves_utils.generate_password(
                core_device_attrs.admin_password_hash, discovered_device.ap_client.mac_address, "viewonly"
            )
    else:
        ...
        while True:
            try:
                await discovered_device.ap_client.request(
                    "GET", "/device", no_log=True, admin_password=discovered_device.admin_password
                )
...
async def _discover(timeout: int) -> list[DiscoveredDevice]:
    ...
    for client in clients:
        logger.debug(
            'discovered AP client "%s" with IP %s and MAC %s', client.hostname, client.ip_address, client.mac_address
        )
