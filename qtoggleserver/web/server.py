...
def _make_routing_table() -> list[URLSpec]:
    ...
    if settings.core.backup_support:
        handlers_list += [URLSpec(r"^/api/backup/endpoints/?$", handlers.BackupEndpointsHandler)]

    # Firmware

    if settings.system.fwupdate.driver:
        handlers_list += [URLSpec(r"^/api/firmware/?$", handlers.FirmwareHandler)]
    ...
    if settings.webhooks.enabled:
        handlers_list += [URLSpec(r"^/api/webhooks/?$", handlers.WebhooksHandler)]

    if settings.core.listen_support:
        handlers_list += [URLSpec(r"^/api/listen/?$", handlers.ListenHandler)]

    # Reverse API calls

    if settings.reverse.enabled:
        handlers_list += [URLSpec(r"^/api/reverse/?$", handlers.ReverseHandler)]

    # System API calls

    if system.conf.can_write_conf_file():
        handlers_list += [URLSpec(r"^/api/system/?$", handlers.SystemHandler)]

    # Introspection API

    if settings.debug:
        handlers_list += [URLSpec(r"^/api/introspect/?$", handlers.IntrospectHandler)]
