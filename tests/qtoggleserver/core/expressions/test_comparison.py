...
async def test_if_boolean(literal_false, literal_true, literal_one, literal_two, dummy_eval_context):
    result = await comparison.IfFunction(
        [literal_false, literal_one, literal_two], role=ROLE_VALUE
    ).eval(dummy_eval_context)
    assert result == 2

    result = await comparison.IfFunction(
        [literal_true, literal_one, literal_two], role=ROLE_VALUE
    ).eval(dummy_eval_context)
    assert result == 1
...
async def test_if_number(literal_zero, literal_one, literal_two, dummy_eval_context):
    result = await comparison.IfFunction(
        [literal_zero, literal_one, literal_two], role=ROLE_VALUE
    ).eval(dummy_eval_context)
    assert result == 2

    result = await comparison.IfFunction(
        [literal_two, literal_one, literal_two], role=ROLE_VALUE
    ).eval(dummy_eval_context)
    assert result == 1
