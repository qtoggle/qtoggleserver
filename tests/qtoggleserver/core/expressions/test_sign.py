...
async def test_abs(literal_two, literal_minus_two):
    result = await sign.AbsFunction([literal_two]).eval(context={})
    assert result == 2

    result = await sign.AbsFunction([literal_minus_two]).eval(context={})
    assert result == 2
...
async def test_sgn(literal_two, literal_minus_two):
    result = await sign.SgnFunction([literal_two]).eval(context={})
    assert result == 1

    result = await sign.SgnFunction([literal_minus_two]).eval(context={})
    assert result == -1
