...
class TestAdd:
    async def test_add_simple(self, literal_one, literal_two, dummy_eval_context):
        result = await arithmetic.AddFunction([literal_one, literal_two], role=Role.VALUE).eval(
            context=dummy_eval_context
        )
        assert result == 3

    async def test_add_multiple(self, literal_one, literal_two, literal_minus_one, literal_ten, dummy_eval_context):
    ...
    async def test_add_boolean(self, literal_one, literal_true, dummy_eval_context):
...
class TestMod:
    async def test_mod_integer(self, literal_ten, literal_two, dummy_eval_context):
        result = await arithmetic.ModFunction([literal_ten, literal_two], role=Role.VALUE).eval(
            context=dummy_eval_context
        )
        assert result == 0

    async def test_mod_float(self, literal_three, literal_two, dummy_eval_context):
        result = await arithmetic.ModFunction([literal_three, literal_two], role=Role.VALUE).eval(
            context=dummy_eval_context
        )
        assert result == 1

    async def test_mod_boolean(self, literal_ten, literal_true, literal_false, dummy_eval_context):
...
class TestPow:
    async def test_pow_integer(self, literal_ten, literal_two, dummy_eval_context):
        result = await arithmetic.PowFunction([literal_ten, literal_two], role=Role.VALUE).eval(
            context=dummy_eval_context
        )
        assert result == 100

    async def test_pow_sqrt(self, literal_sixteen, literal_zero_point_five, dummy_eval_context):
        result = await arithmetic.PowFunction([literal_sixteen, literal_zero_point_five], role=Role.VALUE).eval(
            context=dummy_eval_context
        )
        assert result == 4

    async def test_pow_boolean(self, literal_ten, literal_true, literal_false, dummy_eval_context):
