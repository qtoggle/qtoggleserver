...
async def test_min_simple(literal_one, literal_two):
    result = await aggregation.MinFunction([literal_two, literal_one]).eval()
    assert result == 1


async def test_min_multiple(literal_one, literal_ten, literal_minus_two):
    result = await aggregation.MinFunction([literal_ten, literal_one, literal_minus_two]).eval()
    assert result == -2
...
async def test_max_simple(literal_one, literal_two):
    result = await aggregation.MaxFunction([literal_two, literal_one]).eval()
    assert result == 2


async def test_max_multiple(literal_one, literal_ten, literal_minus_two):
    result = await aggregation.MaxFunction([literal_ten, literal_one, literal_minus_two]).eval()
    assert result == 10
...
async def test_avg_simple(literal_one, literal_two):
    result = await aggregation.AvgFunction([literal_two, literal_one]).eval()
    assert result == 1.5


async def test_avg_multiple(literal_one, literal_ten, literal_minus_two):
    result = await aggregation.AvgFunction([literal_ten, literal_one, literal_minus_two]).eval()
    assert result == 3
