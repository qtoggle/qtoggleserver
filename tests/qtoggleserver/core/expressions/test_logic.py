...
class TestAnd:
    async def test_and_simple(self, literal_false, literal_true, dummy_eval_context):
    ...
    async def test_and_multiple(self, literal_false, literal_true, dummy_eval_context):
    ...
    async def test_and_unavailable(self, literal_false, literal_true, literal_unavailable, dummy_eval_context):
        result = await logic.AndFunction([literal_true, literal_false, literal_unavailable], role=Role.VALUE).eval(
            dummy_eval_context
        )
        assert result == 0

    async def test_and_number(self, literal_zero, literal_ten, literal_true, dummy_eval_context):
    ...
    def test_and_num_args(self):
        with pytest.raises(InvalidNumberOfArguments):
            Function.parse(None, "AND(false)", Role.VALUE, 0)


class TestOr:
    async def test_or_simple(self, literal_false, literal_true, dummy_eval_context):
    ...
    async def test_or_multiple(self, literal_false, literal_true, dummy_eval_context):
    ...
    async def test_or_unavailable(self, literal_false, literal_true, literal_unavailable, dummy_eval_context):
        result = await logic.OrFunction([literal_false, literal_true, literal_unavailable], role=Role.VALUE).eval(
            dummy_eval_context
        )
        assert result == 1

    async def test_or_number(self, literal_zero, literal_ten, literal_true, literal_false, dummy_eval_context):
    ...
    def test_or_num_args(self):
        with pytest.raises(InvalidNumberOfArguments):
            Function.parse(None, "OR(false)", Role.VALUE, 0)


class TestNot:
    async def test_not_simple(self, literal_false, literal_true, dummy_eval_context):
    ...
    async def test_not_number(self, literal_zero, literal_ten, dummy_eval_context):
...
class TestXOr:
    async def test_xor_simple(self, literal_false, literal_true, dummy_eval_context):
    ...
    async def test_xor_number(self, literal_zero, literal_ten, literal_true, literal_false, dummy_eval_context):
