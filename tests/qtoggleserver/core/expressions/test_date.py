...
from qtoggleserver.core.expressions import ROLE_VALUE, Function, date, literalvalues
...
async def test_boy_negative(dummy_local_datetime, dummy_timestamp, local_tz_info, dummy_eval_context):
    result = await date.BOYFunction([literalvalues.LiteralValue(-30, "", ROLE_VALUE)], role=ROLE_VALUE).eval(
        dummy_eval_context
    )
...
async def test_boy_positive(dummy_local_datetime, dummy_timestamp, local_tz_info, dummy_eval_context):
    result = await date.BOYFunction([literalvalues.LiteralValue(100, "", ROLE_VALUE)], role=ROLE_VALUE).eval(
        dummy_eval_context
    )
...
async def test_bom_negative(dummy_local_datetime, dummy_timestamp, local_tz_info, dummy_eval_context):
    result = await date.BOMFunction([literalvalues.LiteralValue(-13, "", ROLE_VALUE)], role=ROLE_VALUE).eval(
        dummy_eval_context
    )
...
async def test_bow_negative(dummy_local_datetime, local_tz_info, dummy_eval_context):
    result = await date.BOWFunction([literalvalues.LiteralValue(-54, "", ROLE_VALUE)], role=ROLE_VALUE).eval(
        dummy_eval_context
    )
...
async def test_bow_sunday(dummy_local_datetime, literal_zero, local_tz_info, dummy_eval_context):
    result = await date.BOWFunction(
        [literal_zero, literalvalues.LiteralValue(6, "", ROLE_VALUE)], role=ROLE_VALUE
    ).eval(dummy_eval_context)
...
async def test_bow_sunday_negative(dummy_local_datetime, local_tz_info, dummy_eval_context):
    result = await date.BOWFunction(
        [literalvalues.LiteralValue(-54, "", ROLE_VALUE), literalvalues.LiteralValue(6, "", ROLE_VALUE)],
        role=ROLE_VALUE,
    ).eval(dummy_eval_context)
