...
class TestNameAttr:
    def test_get_internal(self, mocker):
    ...
    def test_set_internal(self, mocker):
...
class TestFlagsAttr:
    ...
    def test_sequences(self, mocker):
...
class TestPasswordAttrs:
    def test_get_empty(self, mocker):
    ...
    def test_get_not_empty(self, mocker):
    ...
    def test_set_internal_empty(self, mocker):
...
class TestGetSchema:
    ...
    def test_get_loose(self, mocker):
...
class TestGetAttrs:
    ...
    async def test_get_cache_hit(self, mocker):
        """Should return cached attribute values on the second call without invoking any getter again."""

        getter = mock.MagicMock(return_value="dummy1")
        mocker.patch.object(
            device_attrs,
            "get_attrdefs",
            return_value={
        ...
        attrs1 = await device_attrs.get_attrs()
        attrs2 = await device_attrs.get_attrs()

        assert attrs1 == attrs2 == {"name1": "dummy1"}
        getter.assert_called_once()  # getter invoked only on first call; second call returns from cache

    async def test_get_returns_copy(self, mocker):
        """Should return a copy of the cache so that mutations to the returned dict do not corrupt the cache."""
        ...
        attrs2 = await device_attrs.get_attrs()
        assert attrs2["name1"] == "original"

    def test_invalidate_attrs(self):
        """Should set the attrs cache to None so the next `get_attrs` call recomputes attribute values."""

        device_attrs._attrs_cache = {"name1": "dummy1"}
        device_attrs.invalidate_attrs()
        assert device_attrs._attrs_cache is None

    async def test_get_after_invalidate(self, mocker):
        """Should recompute attribute values after `invalidate_attrs` is called, re-invoking getters."""
        ...
        assert attrs1 == {"name1": "value1"}
        assert attrs2 == {"name1": "value2"}
        assert call_count == 2

    async def test_set_invalidates_cache(self, mocker):
        """Should invalidate the attrs cache when `set_attrs` is called, so that subsequent `get_attrs` calls
        recompute attribute values from scratch."""
        ...
        mocker.patch.object(
            ...
            return_value={
                "name1": {
                    "type": "string",
                    "modifiable": True,
                    "getter": mock.MagicMock(return_value="value1"),
                    "setter": mock.MagicMock(),
                },
            },
        )

        invalidate_spy = mocker.spy(device_attrs, "invalidate_attrs")

        await device_attrs.get_attrs()
        assert device_attrs._attrs_cache is not None

        await device_attrs.set_attrs({"name1": "new_value"})
...
class TestSetAttrs:
    async def test_set(self, mocker):
    ...
    async def test_set_reboot(self, mocker):
    ...
    async def test_set_not_modifiable(self, mocker):
